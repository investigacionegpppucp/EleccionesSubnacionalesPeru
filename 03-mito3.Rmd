...
regional_mito3_tipo=regional%>%
  ...
  dplyr::select(Org_pol,Eleccion,Tipo_OP) %>%
  unique%>%
  dplyr::group_by(Eleccion, Tipo_OP)%>% 
  dplyr::summarise(n=n())%>%
  mutate(ratio=scales::percent(n/sum(n),accuracy = 0.1))
```
...
**Porcentaje de organizaciones políticas por tipo y región**
...
**Porcentaje de organizaciones políticas por tipo y region**
...
| **Distrital 2022** |          12          |           115           |           0           |                 0                 |                0                 |            0            |    127    |
...
```{r}
distrital_mito3_mapa$'Número de organizaciones políticas candidatas'=distrital_mito3_mapa$total_org_pol
```

```{r include=FALSE}
mapdist=readOGR("LIMITE_DISTRITAL_2020_INEI.shp") 
geodist22=merge(mapdist,distrital_mito3_mapa,by.x="UBIGEO",by.y="UBIGEO", duplicateGeoms = TRUE)
